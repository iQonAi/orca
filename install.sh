...
resolve_style() {
    ...
    if [-e /dev/tty ] && (: </dev/tty ) 2>/dev/null; then
        ...
        echo "no tty and ORCA_STYLE unset; re-run with ORCA_STYLE=claude or ORCA_STYLE=agents" >&2to3-3.12
        exit 2
    fi
}
...
wire_claude_hook() {
  ...
  backup "$1"; cp "$BACKUP_DIR/settings.json" "$1"  # backup() moved it; work on a restored copy
 tmp=$(mktemp)
